      ...
      <Row className="top-container mt-4">
        <Col xs={6} md={3} >
          <StatCard title="Products"
            bg="bg-success"
            amount={isFetching ? <FetchingSpinner /> : statData?.totalProducts}
            link="/products" />
        </Col>
        <Col xs={6} md={3}>
          <StatCard title="Orders"
            bg="bg-warning"
            amount={isFetching ? <FetchingSpinner /> : (salesData && `$ ${salesData[salesData?.length - 1]?.totalAmount / 100}`)}
            link="/orders" />
        </Col>
        <Col xs={6} md={3}>
          <StatCard title="Customers"
            bg="bg-primary"
            amount={isFetching ? <FetchingSpinner /> : statData?.totalUsers}
            link="/customers" />
        </Col>
        <Col xs={6} md={3}>
          <StatCard title="Mailing List"
            bg="bg-secondary"
            amount={isFetching ? <FetchingSpinner /> : emailData?.length}
            link="/mailinglist" />
        </Col>
      </Row>
