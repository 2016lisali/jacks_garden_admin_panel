...
const MailingList = () => {
  ...
  const ListRow = ({ list }) => (
    <tr className='text-center align-middle'>
      ...
      <td>{list.email}</td>
      <td><input type="checkbox" checked={selectedEmails.includes(list.email)} value={list.email} onChange={handleSelectEmail} /></td>
    </tr>
  )
  const handleSelectEmail = (e) => {
    e.target.checked ? setSelectedEmails([...selectedEmails, e.target.value]) : setSelectedEmails(selectedEmails.filter(email => email !== e.target.value))
  }
  const onSubmit = (data) => {
      ...
      <Row>
        <Col lg className="position-relative">
          ...
          <Table className="shadow-sm mb-2 bg-body rounded text-secondary">
            ...
            <tbody>
              {lists?.slice((currentPage - 1) * 10, currentPage * 10).map(list => <ListRow list={list} key={list.email_id} />)}
            </tbody>
          </Table>
        ...
        <Col lg className='d-flex flex-column justify-content-center'>
          {isSuccess ? (<>
            ...
            <Form ref={form} onSubmit={() => {
              ...
              <FloatingLabel className="mb-3  flex-grow-1" label="Message" style={{ minHeight: '160px' }}>
              ...
              <Form.Check
                ...
                value={allEmails?.map(({ email }) => email).join(",")}
              />
              <Form.Check
                type="radio"
