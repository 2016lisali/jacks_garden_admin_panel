        ...
        <Col md="8">
          ...
          <Table className="shadow-sm border small mb-3 bg-body rounded-3 text-secondary">
            ...
            <tbody>
              {orderDetails?.map(product => (
                <DetailRow data={product} key={product.productId} />
              ))}
            </tbody>
          </Table>
          ...
          <div className="small fw-bolder text-secondary px-2 pb-4" style={{ width: "200px" }}>
            ...
            <div className="w-100 w-lg-25 d-flex justify-content-between py-1">
              ...
              <span>{order.total >= 99 ? order.total : order.total + 9.95}</span>
            </div>
          </div>
        </Col>
        <Col md="4">
          <div className="border rounded-3 text-secondary small p-3 mb-3">
            <p className="fw-bolder pb-1 my-0 small text-decoration-underline">CUSTOMER DETAILS</p>
            <p className="fw-bolder pb-1 my-0">{order.firstName} {order.lastName}</p>
            <p className="pb-1 my-0">{order.email}</p>
          </div>
          <div className="border rounded-3 text-secondary small p-3">
            <p className="fw-bolder pb-1 my-0 small text-decoration-underline">BILING DETAILS</p>
            ...
            <p className="pb-1 my-0">{billingDetails?.postal_code}</p>
          </div>
        </Col>
      </Row>
